...
class Alphabet:
    ...
    def get_index_list(self, instance):
        res = []
        for item in instance:
            try:
                res.append(self.instance2index[str(item)])
            except KeyError:
                print("the ERR key is:\n", item)
                if self.keep_growing:
                    index = self.next_index
                    self.add(instance)
                    res.append(index)
                else:
                    res.append(self.instance2index[self.UNKNOWN])
        return res
